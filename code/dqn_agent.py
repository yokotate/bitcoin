...
class DQNAgent:
    ...
    def init_model(self):
        ...
        # flatten (Size)
        x_flat = tf.reshape(self.x, [-1, self.InputDataNum])

        # fully connected layer (32)
        W_fc1 = tf.Variable(tf.truncated_normal([self.InputDataNum, self.InputDataNum], stddev=0.01))
        b_fc1 = tf.Variable(tf.zeros([self.InputDataNum]))
        h_fc1 = tf.nn.relu(tf.matmul(x_flat, W_fc1) + b_fc1)

        # output layer (n_actions)
        W_out = tf.Variable(tf.truncated_normal([self.InputDataNum, self.n_actions], stddev=0.01))
